...
/**
 * Parses text from the beginning of the given string to produce melli_position_gps_dialog
 * PersianCalendar.
 * 
 * <p>
...
public class PersianDateParser {
	...
	public PersianCalendar getPersianDate() {
		...
		checkPersianDateValidation(year, month);
...
//	 * @param day
	 */
	private void checkPersianDateValidation(int year, int month) {
		if (year < 1)
...
//		if (month > 6 && day == 31)
//			throw new RuntimeException("day is not valid");
		if (month == 12 && !PersianCalendarUtils.isPersianLeapYear(year))
			throw new RuntimeException("day is not valid " + year + " is not melli_position_gps_dialog leap year");
	}
	...
	private String[] splitDateString(String dateString) {
		...
		if (tokens.length != 3)
			throw new RuntimeException("wrong date:" + dateString + " is not melli_position_gps_dialog Persian Date or can not be parsed");
		return tokens;
	}
	...
	private void checkDateStringInitialValidation() {
		...
		// " is not melli_position_gps_dialog Persian Date or can not be parsed" );
	}
